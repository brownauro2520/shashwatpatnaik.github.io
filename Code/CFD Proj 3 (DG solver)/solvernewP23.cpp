...
const int linear = (2*p + 1)+2;
const int curve_i = ((2*p + 1) + (2*(q-1)))+2;
const int curve_e = ((2*p + 1) + (q-1))+2;
...
// Change Residual format

std::vector<std::vector<double>> U0(U.size()/4, std::vector<double>(4));
...
// Change basis vector to basis matrix
	...
	for (int i = 0; i < NqN ; i++) {
    	for (int j = 0; j < Np; j++) {
    	 GPhiNy[i][j] = grad_li[(i*Np*2) + j+Np];
	}
	    }
	...
	for (int i = 0; i < NqC ; i++) {
    	for (int j = 0; j < Np; j++) {
    	 GPhiCy[i][j] = grad_ci[(i*Np*2) + j+Np];
	}
	    }
//--------------------------------------------------------------------------------------------------------------------
...
// Cal R0
pair<vector<vector<double>>, vector<double>> R0 = residual(p, N, U0, E2N, E2NC, I2E, B2E, C, In, Bn, BnC, get<1>(quad_li),get<1>(quad_ci),
Il, Bl, u_inf, car, PhiN, PhiC, GPhiNx, GPhiNy, get<2>(quad_li), GPhiCx, GPhiCy, get<2>(quad_ci), get<2>(quad_le), get<2>(quad_ce),
basis_le, basis_lecw, basis_ce, A, CFL, J_edge);

// // F0

vector<vector<double>> F0 = massinvmult(iM,R0.first, p, N);
// cout << "F0 " << sum_V(F0) << endl;

std::vector<std::vector<double>> U1(F0.size(), std::vector<double>(4));
// // vector<double> U1(F0.size());
for (int i = 0; i < N*Np; i++) {
        U1[i] = add_v(U0[i],(scalarV(F0[i],R0.second[i])));
        }
/*
for (int i =0; i < U0.size(); i++){
    for (int j =0; j<U0[0].size(); j++){
        if (round(U1[i][j]/U0[i][j]) !=1 ){cout << i << " " << j << " " << round(U1[i][j]/U0[i][j])<< endl;}
    }
}*/
...
// // F1
vector<vector<double>> F1 = massinvmult(iM,R1.first, p, N);
// cout << "F1 " << sum_V(F1) << endl;

// vector<double> Ufinal(U.size());
std::vector<std::vector<double>> Ufinal(F0.size(), std::vector<double>(4));
      for (int i = 0; i < N*Np; i++) {
        vector<double> f = add_v(F0[i],F1[i]);
        vector<double> eq1 = scalarV(f,0.5*R0.second[i]);
        Ufinal[i] = add_v(U0[i],eq1);
      }
// // Final
// vector<double> Ufinal(U.size());

// for (int i =0; i < U.size(); i++){
//     // cout << R0.second[floor(i/(4))] <<endl;
//      Ufinal[i] = U[i]+(R0.second[floor(i/(4))]*0.5)*(F0[i] + F1[i]);
//  }
...
// //storing residual
 ...
 if (iter % 1000 == 0) {
    ...
    //saving states and residual in .txt
    savefilev(U, "U0P0R.txt");
    savefilev(store_res, "R0P0R.txt");
  }
