...
if __name__ == "__main__":
    ...
    for i, n in enumerate(scale):
        ...
        # Extract the iteration and error history
        iter = outputs['iteration']
        iterations_func = np.arange(0, iter + 1, 1)
        error_history_func = outputs['error history']
        if error_history_func[-1] == 0:
            error_history_func[-1] = 1e-16
        
        # Plot error history for the custom function in the corresponding subplot
        ax = axes[i]
        ax.semilogy(iterations_func, error_history_func, 'r--', label=outputs["namesearchdir"] + '-' + outputs["namelinesearch"])
        ax.set_xlabel('Iteration', fontname='Times New Roman', fontsize=14)
        ax.set_title('Convergence Plot of ' + func_name + r' with a $x_0$ = ' + str(x0), fontname='Times New Roman', fontsize=14)
        ax.set_ylabel(r"$|∇f(x)||_∞$", fontsize=14)
        ...
        # Extract the iteration and error history for scipy
        iter = outputs['iteration']
        iterations_scipy = np.arange(0, iter + 1, 1)
        error_history_scipy = outputs['error history']
        
        # Plot error history for scipy function in the corresponding subplot
        ax.semilogy(iterations_scipy, error_history_scipy, 'b--', label='Scipy Minimize - BFGS')
        ax.legend()
